...
*
* @package phpBB3
* @version $Id: prime_notify.php,v 1.0.3 2008/08/18 22:43:00 primehalo Exp $
...
if (!defined('INCLUDES_PRIME_NOTIFY'))
{
	...
	* Options
	*/
	define('PRIME_NOTIFY_POST_ENABLED', true);	// Enable or disable this MOD for post notifications.
	define('PRIME_NOTIFY_PM_ENABLED', true);	// Enable or disable this MOD for private message notifications.
	define('PRIME_NOTIFY_BBCODES', true);		// Having BBCodes in the email will help show how the message is supposed to be formatted.
	...
	*/
	class prime_notify
	{
		...
		*/
		...
		{
			$this->user_enabled = (!isset($row['user_notify_content']) || (PRIME_NOTIFY_USER_CHOICE && !empty($row['user_notify_content'])));
			if ($this->enabled && $this->user_enabled)
			{
		...
		*/
		function alter_post_sql(&$sql)
		{
			if ($this->enabled)
			{
				// Always notify, so don't if a notification was already sent
				if (PRIME_NOTIFY_ALWAYS)
				{
